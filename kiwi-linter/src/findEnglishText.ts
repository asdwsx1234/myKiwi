...
import { ENG_REG as DOUBLE_BYTE_REGEX } from './const';
...
function isImport(node) {
  ...
  if (node.parent.kind === ts.SyntaxKind.ImportDeclaration || node.parent['importClause']) {
    return true;
  }
  return false;
}

const classNames = ['className', 'styleName'];

function isClassName(node) {
  if (node.parent && node.parent.name && classNames.includes(node.parent.name.escapedText)) {
    return true;
  }
  return false;
}
  ...
  function visit(node: ts.Node) {
    switch (node.kind) {
      case ts.SyntaxKind.StringLiteral: {
        if (isImport(node) || isClassName(node)) {
          break;
        }
        /** 判断 Ts 中的字符串含有英文 */
 ...
 * @param code
 */
export function findEnglishText(code: string, fileName: string) {
